...
// ==============================================================
`timescale 1ns/1ps
module getFilterOut_AXILiteS_s_axi
#(parameter
...
localparam
    ...
    ADDR_IER                = 9'h008,
    ADDR_ISR                = 9'h00c,
    ADDR_DEPTH_DATA_0       = 9'h080,
    ADDR_DEPTH_CTRL         = 9'h084,
    ADDR_OUT_CORRECT_DATA_0 = 9'h100,
    ADDR_OUT_CORRECT_CTRL   = 9'h104,
    ADDR_INPUTS_BASE        = 9'h040,
    ADDR_INPUTS_HIGH        = 9'h07f,
    ADDR_OUT_R_BASE         = 9'h0c0,
    ADDR_OUT_R_HIGH         = 9'h0ff,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [15:0]                   int_depth = 'b0;
    reg  [0:0]                    int_out_correct = 'b0;
    reg                           int_out_correct_ap_vld;
    // memory signals
    wire [3:0]                    int_inputs_address0;
    wire                          int_inputs_ce0;
    wire                          int_inputs_we0;
    wire [3:0]                    int_inputs_be0;
    wire [31:0]                   int_inputs_d0;
    wire [31:0]                   int_inputs_q0;
    wire [3:0]                    int_inputs_address1;
    wire                          int_inputs_ce1;
    wire                          int_inputs_we1;
...
//------------------------Instantiation------------------
// int_inputs
getFilterOut_AXILiteS_s_axi_ram #(
    .BYTES    ( 4 ),
    .DEPTH    ( 10 )
) int_inputs (
    .clk0     ( ACLK ),
...
assign RVALID  = (rstate == RDDATA) & !int_inputs_read & !int_out_r_read;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_DEPTH_DATA_0: begin
                    rdata <= int_depth[15:0];
                end
                ADDR_OUT_CORRECT_DATA_0: begin
                    rdata <= int_out_correct[0:0];
                end
                ADDR_OUT_CORRECT_CTRL: begin
                    rdata[0] <= int_out_correct_ap_vld;
                end
            endcase
        end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_DEPTH_DATA_0)
            int_depth[15:0] <= (WDATA[31:0] & wmask) | (int_depth[15:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (out_correct_ap_vld)
            int_out_correct <= out_correct;
    end
end

// int_out_correct_ap_vld
always @(posedge ACLK) begin
    if (ARESET)
        int_out_correct_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (out_correct_ap_vld)
            int_out_correct_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_OUT_CORRECT_CTRL)
            int_out_correct_ap_vld <= 1'b0; // clear on read
    end
end
...
assign int_inputs_address1 = ar_hs? raddr[5:2] : waddr[5:2];
...
assign int_out_r_address1  = ar_hs? raddr[5:2] : waddr[5:2];
...
module getFilterOut_AXILiteS_s_axi_ram
#(parameter
    BYTES  = 4,
    DEPTH  = 256,
    AWIDTH = log2(DEPTH)
