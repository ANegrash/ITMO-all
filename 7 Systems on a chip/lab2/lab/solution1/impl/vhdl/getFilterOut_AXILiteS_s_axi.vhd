...
entity getFilterOut_AXILiteS_s_axi is
generic (
...
end entity getFilterOut_AXILiteS_s_axi;
...
architecture behave of getFilterOut_AXILiteS_s_axi is
    ...
    constant ADDR_DEPTH_DATA_0       : INTEGER := 16#080#;
    constant ADDR_DEPTH_CTRL         : INTEGER := 16#084#;
    constant ADDR_OUT_CORRECT_DATA_0 : INTEGER := 16#100#;
    constant ADDR_OUT_CORRECT_CTRL   : INTEGER := 16#104#;
    constant ADDR_INPUTS_BASE        : INTEGER := 16#040#;
    constant ADDR_INPUTS_HIGH        : INTEGER := 16#07f#;
    constant ADDR_OUT_R_BASE         : INTEGER := 16#0c0#;
    constant ADDR_OUT_R_HIGH         : INTEGER := 16#0ff#;
    ...
    signal int_depth           : UNSIGNED(15 downto 0) := (others => '0');
    signal int_out_correct     : UNSIGNED(0 downto 0) := (others => '0');
    signal int_out_correct_ap_vld : STD_LOGIC;
    -- memory signals
    signal int_inputs_address0 : UNSIGNED(3 downto 0);
    ...
    signal int_inputs_address1 : UNSIGNED(3 downto 0);
    ...
    signal int_out_r_address0  : UNSIGNED(3 downto 0);
    ...
    signal int_out_r_address1  : UNSIGNED(3 downto 0);
    ...
    component getFilterOut_AXILiteS_s_axi_ram is
        generic (
            BYTES   : INTEGER :=4;
            DEPTH   : INTEGER :=256;
            AWIDTH  : INTEGER :=8);
    ...
    end component getFilterOut_AXILiteS_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_inputs
int_inputs : getFilterOut_AXILiteS_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 10,
     AWIDTH   => log2(10))
port map (
     ...
     q1       => int_inputs_q1);
-- int_out_r
int_out_r : getFilterOut_AXILiteS_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 10,
     AWIDTH   => log2(10))
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_inputs_read = '0') and (int_out_r_read = '0') else '0';
    RVALID    <= RVALID_t;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_DEPTH_DATA_0 =>
                        rdata_data <= RESIZE(int_depth(15 downto 0), 32);
                    when ADDR_OUT_CORRECT_DATA_0 =>
                        rdata_data <= RESIZE(int_out_correct(0 downto 0), 32);
                    when ADDR_OUT_CORRECT_CTRL =>
                        rdata_data <= (0 => int_out_correct_ap_vld, others => '0');
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
...
-- ----------------------- Register logic ----------------
    ...
    depth                <= STD_LOGIC_VECTOR(int_depth);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_DEPTH_DATA_0) then
                    int_depth(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_depth(15 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_out_correct <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (out_correct_ap_vld = '1') then
                    int_out_correct <= UNSIGNED(out_correct); -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_out_correct_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (out_correct_ap_vld = '1') then
                    int_out_correct_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_OUT_CORRECT_CTRL) then
                    int_out_correct_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    ...
    int_inputs_address1  <= raddr(5 downto 2) when ar_hs = '1' else waddr(5 downto 2);
    ...
    int_out_r_address1   <= raddr(5 downto 2) when ar_hs = '1' else waddr(5 downto 2);
...
entity getFilterOut_AXILiteS_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity getFilterOut_AXILiteS_s_axi_ram;

architecture behave of getFilterOut_AXILiteS_s_axi_ram is
